...
var source = new EventSource('http://104.154.205.106/events');
...
function dologin() {
	...
	if( !check_name( $("#name").val() ) ) {
        ...
        socket = io("http://104.154.205.106")
...
socket.on("LOGIN_RESPONSE", function(response){    
    setup()
    instantiate(userName, true);
    response.forEach(function(name)
    {
        instantiate(name, false);
    })
})
...
socket.on("LOGIN", function(response){ 
    log(response.name,"Just logged in") 
    instantiate(response.name, false);
})
...
function dopost () {
	...
	if( $("#input").val() != "" ) {
        ...
        userData = {name: userName, msg : userMessage}
        socket.emit("POST", userData);
...
socket.on("POST", function(response){
    log(response.name, response.msg)      
})
...
function mainLoop() {
    ...
    userData = {name: userName, left: posx, top: posy};
...
socket.on("MOVE", function(response){
    move(response.name, response.left, response.top);     
})
