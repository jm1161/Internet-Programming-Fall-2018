 ...
 *
 * @method     ChildPersonQuery leftJoinPhoneNumbers($relationAlias = null) Adds a LEFT JOIN clause to the query using the PhoneNumbers relation
 * @method     ChildPersonQuery rightJoinPhoneNumbers($relationAlias = null) Adds a RIGHT JOIN clause to the query using the PhoneNumbers relation
 * @method     ChildPersonQuery innerJoinPhoneNumbers($relationAlias = null) Adds a INNER JOIN clause to the query using the PhoneNumbers relation
 *
 * @method     ChildPersonQuery joinWithPhoneNumbers($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the PhoneNumbers relation
 *
 * @method     ChildPersonQuery leftJoinWithPhoneNumbers() Adds a LEFT JOIN clause and with to the query using the PhoneNumbers relation
 * @method     ChildPersonQuery rightJoinWithPhoneNumbers() Adds a RIGHT JOIN clause and with to the query using the PhoneNumbers relation
 * @method     ChildPersonQuery innerJoinWithPhoneNumbers() Adds a INNER JOIN clause and with to the query using the PhoneNumbers relation
 *
 * @method     \PhoneNumbersQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class PersonQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related \PhoneNumbers object
     *
     * @param \PhoneNumbers|ObjectCollection $phoneNumbers the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildPersonQuery The current query, for fluid interface
     */
    public function filterByPhoneNumbers($phoneNumbers, $comparison = null)
    {
        if ($phoneNumbers instanceof \PhoneNumbers) {
            return $this
                ->addUsingAlias(PersonTableMap::COL_ID, $phoneNumbers->getPersonId(), $comparison);
        } elseif ($phoneNumbers instanceof ObjectCollection) {
            return $this
                ->usePhoneNumbersQuery()
                ->filterByPrimaryKeys($phoneNumbers->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByPhoneNumbers() only accepts arguments of type \PhoneNumbers or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the PhoneNumbers relation
     *
     ...
     * @return $this|ChildPersonQuery The current query, for fluid interface
     */
    public function joinPhoneNumbers($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('PhoneNumbers');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'PhoneNumbers');
        }
    ...
    /**
     * Use the PhoneNumbers relation PhoneNumbers object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \PhoneNumbersQuery A secondary query class using the current class as primary query
     */
    public function usePhoneNumbersQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinPhoneNumbers($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'PhoneNumbers', '\PhoneNumbersQuery');
    }
